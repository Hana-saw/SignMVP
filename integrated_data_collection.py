...
def integrated_data_collection(target_labels, samples_per_label=10):
    """
    データ収集とランドマーク保存を統合
    """
    # 現在のデータ数を確認
        ...
        for i, label in enumerate(target_labels):
            if key == ord(str(i+1)):
                if collected_counts[label] < samples_per_label:
                    ...
                    # labels.csvに追加
                    with open("labels.csv", 'a', newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
    ...
    print("labels.csv と landmarks/ フォルダの両方に保存されました")
...
if __name__ == "__main__":
    ...
    input("Enterキーを押して開始...")
    integrated_data_collection(target_labels, samples_per_label)
